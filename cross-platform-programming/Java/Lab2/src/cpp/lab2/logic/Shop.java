...
public class Shop {
    ...
    public void makeOrder(String orderName, int amount) {
        Order order = clientBase.getCurrentClient().makeOrder(orderName,
                amount);
        clientBase.addOrderInfo(order.getId(), order.getName(),
                order.getAmount(), order.getPrice());
        clientBase.updateCurrentClientMoney(clientBase.getCurrentClient()
                                                      .getMoney());
    }
